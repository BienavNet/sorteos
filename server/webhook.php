...
$access_token = "TEST-1195720665909450-042913-736ac513493e15375aa70b9bbe45730c-1186797523";
...
try {
    ...
    if (key_exists('data', $incoming_data)) {
                                ...
                                if (!$req["error"]) {
                                    ...
                                    if (!$req["error"]) {
                                        ...
                                        if ($data["payer"]["cedula"] == "1129564256")
                                            ...
                                            $tickets = getTickets($can_tickets);
        ...
        echo "No se pudo establecer conexion con mercado pago";
    }
} catch (Exception $e) {
...
function sendEmail($mail, $body, $email, $alt_body)
{
    try {
        ...
        $mail->Port = 465;
        $mail->setFrom("testproof490@gmail.com", "Tornilujos la 40");
        $mail->addAddress("sivirpax982@gmail.com");
        $mail->isHTML(true);
        $mail->Subject = "Entrega de Boletos";
        $mail->Body = $body;
        $mail->AltBody = '';
...
function getTickets($cant = 1)
{

    try {

        #print_r($used_tickets);
        for ($i = 0; $i < $cant; $i++) {
            $db = new Database([]);
            ...
            $tickets = [];
            $used_tickets = $formated_tickets;
            print_r(count($used_tickets));
            $random = random_int(0, count($used_tickets) - 1);
            array_push($tickets, str_pad($used_tickets[$random], 4, "0", STR_PAD_LEFT));
            $db->ticketSold([$used_tickets[$random]]);
            print_r(count($db->noUsedTickets()));
        }
        return $tickets;
    } catch (\Throwable $th) {
